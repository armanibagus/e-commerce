...
export default {
  ...
  actions: {
    async getRegisterData({ commit, dispatch }, payload) {
      const APIkey  = "AIzaSyDITSXbtiW80io9dYXUpeLUM3NMcfYuqGI";
    ...
    async addNewUser({ commit, state }, payload) {
      ...
      try {
        const { data } = await axios.post(`${databaseUrl}users.json?auth=${state.token}`, payload);
    ...
    async getUser({ commit }, payload) {
      ...
      try {
        const { data } = await axios.get(`${databaseUrl}users.json`);
        for (let key in data) {
